<template>
  <div class="RESTfUl-index">
    <div style="margin-left: 20px;">
      <div><a href="https://fqihrx37dhp.feishu.cn/docx/Nw45d16hCo05bUxbqp1cx970nkd#part-W4PEd7Iw6oE3v0x0bjBcD6Ecncc"
              target="_blank">TTS-V2-RESTful-文档地址</a></div>
    </div>
    <div class="page-main">
      <el-form
          ref="ruleFormRef"
      ...
      >
        ...
        <div>
          ...
          <audio :src="audioUrl" controls autoplay style="margin-left: 120px;"></audio>
        </div>
      </el-form>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      formSize: "small",
      client_secret: '', // 获取 client_secret,client_id 开放平台【https://ai.data-baker.com/】语音合成 -> 短文本合成 -> 授权管理
      client_id: '', // 获取 client_secret,client_id 开放平台【https://ai.data-baker.com/】语音合成 -> 短文本合成 -> 授权管理
      tokenUrl: "https://openapi.data-baker.com/oauth/2.0/token",
      ttsUrl: "https://openapi.data-baker.com/tts"
    }
  },
  mounted() {
  ...
  methods: {
    async play() {
      try {
        // 请求token
        let url = `${this.tokenUrl}?grant_type=client_credentials&client_secret=${this.client_secret}&client_id=${this.client_id}`;
        let res = await request(url, {
          method: 'get',
        })
        // 请求音频
        let audioRes = await request(this.ttsUrl, {
          method: 'get',
          responseType: 'blob',
          params: {
            access_token: res.access_token,
            text: this.ruleForm.text,
            voice_name: this.ruleForm.voice_name,
            language: this.ruleForm.language,
            speed: this.ruleForm.speed,
            volume: this.ruleForm.volume,
            pitch: this.ruleForm.pitch,
            sample_rate: this.ruleForm.sample_rate,
            timestamp: this.ruleForm.timestamp,
            silence: this.ruleForm.silence,
            emo_type: this.ruleForm.emo_type,
            emo_intensity: this.ruleForm.emo_intensity,
            domain: '1',
            version: '2.1',
            audio_fmt: this.ruleForm.audio_fmt
          }
        })
        console.log(audioRes.type)
        if (audioRes.type === 'audio/wav' || audioRes.type === 'audio/mp3') {
          this.audioUrl = window.URL.createObjectURL(audioRes)
          return
        }
        // 解析blob
        ...
        reader.onload = function(event) {
          ...
          console.log(content);
          alert(`err_no:${content.err_no}, err_msg:${content.err_msg}`)
        };

        reader.readAsText(audioRes);
        // console.log(window.URL.createObjectURL(audioRes));
      } catch (e) {
        console.log(e)
        // 如果是401
        if (e.response.status === 401) {
          alert('请填写有效的client_secret和client_id')
        }
      }
    },
    download() {
